...
=== "**Downloading method 1: Using `wget` to install binary packages**"
     ...
     unzip -d matrixone/ mo-v3.0.10-darwin-x86_64.zip
     ```
     ...
     unzip -d matrixone/ mo-v3.0.10-darwin-arm64.zip
     ```
...
=== "**Downloading method 3: Go to the page and download**"
     ...
     If you want a more intuitive way to download the page, go to the [version 3.0.10](https://github.com/matrixorigin/matrixone/releases/tag/v3.0.10), pull down to find the **Assets** column, and click the installation package *mo-v3.0.10-darwin-x86_64.zip* or *mo-v3.0.10-darwin-arm64.zip* can be downloaded.
...
````
mo_ctl set_conf MO_PATH="/yourpath/mo-v3.0.10-xx-xx" # Set the MO_PATH to the directory where the binary files are extracted
mo_ctl set_conf MO_CONF_FILE="/yourpath/mo-v3.0.10-xx-xx/etc/launch/launch.toml" # Set the MO_CONF_FILE path
...
If the operation is regular, the following log will appear. The relevant operation logs of MatrixOne will be in `/yourpath/mo-v3.0.10-xx-xx/matrixone/logs/ .
